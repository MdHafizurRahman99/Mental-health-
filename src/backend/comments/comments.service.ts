...
export class CommentsService {
  ...
  async create(createCommentDto: CreateCommentDto): Promise<Comment> {
    ...
    await this.postsService.incrementCommentsCount(
  ...
  async findReplies(
    ...
    const [replies, total] = await Promise.all([
      this.commentModel
        ...
        .exec(),
      this.commentModel
        .countDocuments({ parentCommentId: new Types.ObjectId(commentId) })
        .exec(),
    ]);
  ...
  async incrementReactionsCount(commentId: string): Promise<void> {
    await this.commentModel
      .findByIdAndUpdate(commentId, { $inc: { reactionsCount: 1 } })
      .exec();
  }
